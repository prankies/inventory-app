...
const InventoryApp = () => {
  ...
  const shareDailyWhatsApp = async () => {
    ...
    // Hi-DPI canvas for crisp, large text
    const W = 920, pad = 40, rowH = 56, headH = 170, scale = 2;
    const H = headH + 90 + list.length * rowH + 60;
    ...
    x.font = 'bold 40px Arial'; x.fillText('Daily Stock Received', pad, 62);
    x.font = '24px Arial'; x.fillStyle = 'rgba(255,255,255,.9)';
    x.fillText('Goyal Printing & Converting Industries', pad, 100);
    x.font = 'bold 22px Arial'; x.fillStyle = 'rgba(255,255,255,.85)';
    x.fillText(`${list.length} item(s) received`, pad, 138);
    // Date — top right, large
    x.textAlign = 'right'; x.fillStyle = '#ffffff';
    x.font = 'bold 34px Arial'; x.fillText(dateStr, W - pad, 66);
    x.textAlign = 'left';
    // Column headers
    let y = headH + 56;
    x.fillStyle = '#64748b'; x.font = 'bold 20px Arial';
    x.fillText('#  ITEM', pad, y);
    x.fillText('QTY', 560, y);
    x.fillText('KEPT AT', 720, y);
    y += 16; x.strokeStyle = '#cbd5e1'; x.lineWidth = 1.5;
    x.beginPath(); x.moveTo(pad, y); x.lineTo(W - pad, y); x.stroke();
    y += 40;
    list.forEach((r, i) => {
      x.fillStyle = '#0f172a'; x.font = '23px Arial';
      x.fillText(`${i + 1}. ${r.name}`.substring(0, 34), pad, y);
      x.fillStyle = '#0f172a'; x.font = 'bold 23px Arial';
      x.fillText(`${r.quantity} ${r.unit || ''}`, 560, y);
      x.fillStyle = '#4338ca'; x.font = 'bold 23px Arial';
      x.fillText(r.godown, 720, y);
      x.strokeStyle = '#eef1f6'; x.lineWidth = 1;
      x.beginPath(); x.moveTo(pad, y + 18); x.lineTo(W - pad, y + 18); x.stroke();
      y += rowH;
    });
    x.fillStyle = '#94a3b8'; x.font = '18px Arial';
