...
const InventoryApp = () => {
  ...
  const shareDailyWhatsApp = async () => {
    ...
    // Hi-DPI canvas, two lines per item so text stays large & readable on WhatsApp.
    const W = 860, pad = 44, blockH = 150, headH = 250, scale = 2;
    const H = headH + 40 + list.length * blockH + 70;
    ...
    x.font = 'bold 52px Arial'; x.fillText('Daily Stock Received', pad, 88);
    x.font = '30px Arial'; x.fillStyle = 'rgba(255,255,255,.9)';
    x.fillText('Goyal Printing & Converting Industries', pad, 138);
    x.font = 'bold 30px Arial'; x.fillStyle = 'rgba(255,255,255,.85)';
    x.fillText(`${list.length} item(s) received`, pad, 200);
    // Date — top right, large
    x.textAlign = 'right'; x.fillStyle = '#ffffff';
    x.font = 'bold 56px Arial'; x.fillText(dateStr, W - pad, 96);
    x.textAlign = 'left';
    // Item blocks — big name line, then qty • location line
    let y = headH + 74;
    list.forEach((r, i) => {
      x.fillStyle = '#0f172a'; x.font = 'bold 44px Arial';
      x.fillText(`${i + 1}. ${r.name}`.substring(0, 26), pad, y);
      x.font = '38px Arial';
      x.fillStyle = '#334155';
      const qtyTxt = `${r.quantity} ${r.unit || ''}`.trim();
      x.fillText(qtyTxt, pad + 34, y + 52);
      const qtyW = x.measureText(qtyTxt).width;
      x.fillStyle = '#94a3b8'; x.fillText('•', pad + 34 + qtyW + 22, y + 52);
      x.fillStyle = '#4338ca'; x.font = 'bold 38px Arial';
      x.fillText(String(r.godown), pad + 34 + qtyW + 52, y + 52);
      x.strokeStyle = '#eef1f6'; x.lineWidth = 2;
      x.beginPath(); x.moveTo(pad, y + 84); x.lineTo(W - pad, y + 84); x.stroke();
      y += blockH;
    });
    x.fillStyle = '#94a3b8'; x.font = '26px Arial';
    x.fillText('Generated by Inventory Hub • stock.gpci.in', pad, H - 34);
